...
- Apply Fix: product type + material are auto-fixable via real Shopify `productUpdate` (title rewrite for material)
- After any successful fix, Guardian re-fetches and re-analyzes (no optimistic local MATCH patch)
- Harden applyFix GraphQL error handling; prevent double-submit while applying
- Visual Listing Guardian V2: multi-signal consistency (color, product type, material)
- Conservative color + product-type normalization (exact match, no substring)
- SignalResult vs ConsistencyIssue separation; presentation-only Guardian UI
- Dynamic confidence redistribution; image quality as gate (not a weight)
- Preserve pixel+vision color confidence blending (`blendColorConfidence`)
- Visual Listing Guardian (Sellenvo Vision): product list + Guardian route
- Groq `qwen/qwen3.6-27b` vision analysis with Zod validation
- Consistency engine (MATCH / MISMATCH / UNCERTAIN) with derived confidence
- Optional colorthief/sharp pixel color signal
- Apply Fix via `productOptionUpdate` + re-fetch verification
